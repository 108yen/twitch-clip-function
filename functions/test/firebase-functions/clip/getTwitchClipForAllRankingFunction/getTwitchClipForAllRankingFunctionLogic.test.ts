...
describe(`getTwitchClipForAllRankingFunctionLogicのテスト`, () => {
    ...
    beforeAll(async () => {
        ...
        getTwitchClipForAllRankingFunctionLogic = await GetTwitchClipForAllRankingFunctionLogic.init();
    })
    ...
    test(`getStreamersのテスト:firestoreエラー`, async () => {
        ...
        await expect(getTwitchClipForAllRankingFunctionLogic.getStreamers()).rejects.toThrowError();
        expect(getStreamersSpy).toHaveBeenCalled();
    }, 100000)
            ...
            if (Object.prototype.hasOwnProperty.call(updateClipDocSpy.mock.calls, key)) {
                ...
                for (const [_, value] of args[1].clipsMap) {
                    ...
                    for (let index = 0; index < value.length - 1; index++) {
                        expect(value[index].view_count!).toBeGreaterThanOrEqual(value[index + 1].view_count!);
                    }
                }
            }
        }
    ...
    test(`getClipForEeachStreamersのテスト:axiosエラー`, async () => {
        ...
        await expect(getTwitchClipForAllRankingFunctionLogic.getClipForEeachStreamers(streamer))
