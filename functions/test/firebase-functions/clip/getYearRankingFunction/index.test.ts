...
describe(`getYearRankingFunctionのテスト`, () => {
    ...
    test(`更新`, async () => {
        ...
        for (const key in streamers) {
            ...
            for (const [_, period] of removePeriods) {
            ...
            for (const [period, clips] of clipDoc.clipsMap) {
                ...
                for (const key_j in clips) {
                    ...
                    if (!isNaN(Number(period))) {
                        ...
                        expect(new Date(clip.created_at!).getTime()).toBeGreaterThanOrEqual(started_at.getTime());
                        expect(new Date(clip.created_at!).getTime()).toBeLessThanOrEqual(ended_at.getTime());
                    }
                }
                clipDoc.clipsMap.delete(period);
